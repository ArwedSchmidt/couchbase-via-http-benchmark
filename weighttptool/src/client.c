...
memset(&tmpBuff, 0, 512);
	...
	switch (client->state) {
		...
		case CLIENT_WRITING:
                       ...
                       // TODO: need make a copy of config->request! add  counter to that one
	// besser. 10.000.000 nur durch counter ersetzen an fester position
// stringify new counter


memset(&newBuff, 0, 32);
nowVal = client->worker->config->atomicCounter;
client->worker->config->atomicCounter++;
sprintf(&(newBuff[0]), "%d", nowVal);
...
replaceAdr = (((char*)config->request) + 60);
memcpy(replaceAdr, &(newBuff[0]), 8);

//printf("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n");
//printf("%s", config->request);
